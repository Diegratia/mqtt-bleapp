...
// Fungsi untuk menghasilkan area deteksi (±3 piksel)
  ...
  linePixels.forEach(([x, y]) => {
    for (let dx = -3; dx <= 3; dx++) {
      for (let dy = -3; dy <= 3; dy++) {
        ...
        if (distToLine <= 3) {
          detectionArea.add(JSON.stringify([newX, newY]));
        }
      }
    }
  });

  return Array.from(detectionArea).map((str) => JSON.parse(str));
}

// JSON statis untuk area deteksi
...
function drawDetectionArea() {
  detectionArea.forEach(([x, y]) => {
    ctx.fillStyle = "rgba(246, 0, 0, 0.2)";
...
function getTagPosition(calc_dist1, calc_dist2) {
  ...
  if (
    !detectionArea.some(
      ([dx, dy]) => dx === nearestPixel[0] && dy === nearestPixel[1]
    )
  ) {
    return null;
  }
